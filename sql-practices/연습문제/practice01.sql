...
select count(salary) as '근무중인 직원수'
	from salaries;
    
-- 문제5.
-- 부서는 총 몇 개가 있나요? 디스 머시기랑 카운트 같이써라

select distinct(count(dept_no)) as '부서 개수'
	from departments;

-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외) 카운트 뎁트매니저
...
select emp_no as 사번, title as 직책, from_date as '직책 시작', to_date as '직책 종료'
	from titles
    where emp_no='13250'
